...
*Setup instructions for a Gitian build of Arena Core using a Debian VM or physical system.*
...
- [Building Arena Core](#building-arena-core)
...
Clone the git repositories for Arena Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/arenacoin/arenacoin
```
...
in the Arena Core repository under 'Fetch and build inputs' to install sources which require
...
To build Arena Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Arena Core repository.
...
```bash
    ...
    From https://github.com/arenacoin/arenacoin
...
For example:
```bash
URL=https://github.com/crowning-/arena.git
...
and the arena git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../arena/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/arenacoin.git
SIGPATH=/some/root/path/arenacoin-detached-sigs.git

./bin/gbuild --url arena=${BTCPATH},signature=${SIGPATH} ../arena/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/arena-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/arena-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/arena-osx-build.assert
```
...
[arena/gitian.sigs](https://github.com/arenacoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
