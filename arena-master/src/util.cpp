...
const char * const BITCOIN_CONF_FILENAME = "arena.conf";
const char * const BITCOIN_PID_FILENAME = "arenad.pid";
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ArenaCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ArenaCore
    // Mac: ~/Library/Application Support/ArenaCore
    // Unix: ~/.arenacore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ArenaCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ArenaCore";
#else
    // Unix
    return pathRet / ".arenacore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty arena.conf if it does not excist
        ...
        if (configFile != NULL) {
        	...
        	LogPrintf("arena.conf file not found or can't be created\n");
